#### R code for Evidence of vocal performance constraints in a female non-human primate 
#### Sound files available at: https://github.com/DenaJGibbon/Performance-Constraints

#### Part 1 Bandwidth calculation (line 31)
#### Part 2 Signal-to-noise ratio calculation (line 150)
#### Part 3 Quantile Regression and Model selection code (line 325)
...
setwd("/Users/denasmacbook/Downloads/Performance-Constraints-master")


#### Part 1 Bandwidth calculation  #### 

#Set input directory to read in .wav files
input.dir <-"/Users/denasmacbook/Downloads/Performance-Constraints-master/TrillNotes"
...
# Run the loop to calculate bandwidth
for (j in 1:length(filehandles)) { 
      ...
      frequency <- rollapply((seq(from=1,to=nrow(psd),by=1)),width=100, by=50,median) # calculate the moving average
...
#### Part 2 Signal-to-noise ratio calculation #### 
    ...
    # Read in data file
    snr.data <- read.csv("/Users/denasmacbook/Downloads/Performance-Constraints-master/snr.data.subset.csv",header = T)
    str(snr.data)
    
    # Create index of calls for the loop

    input.dir.new <- "/Users/denasmacbook/Downloads/Performance-Constraints-master/long_wav_files_for_snr"
    ...
    for (j in 1:length(L.snr)) { tryCatch({
      ...
      great.call <- str_split_fixed(snr.file, pattern="/", n=8)[[7]]
    ...
    ##Set wd to location of shorter wave files and Raven .txt files
    setwd("/Users/denasmacbook/Downloads/Performance-Constraints-master/wav_and_txt_files")
    
    ## Create index of great calls for the loop below
    great.call.index <- unique(snr.data$great.call)
      
    ##Loop to calculate SNR
    note.snr <- lapply(1:length(great.call.index), function(j){ tryCatch({
      great.call.in <- great.call.index[j]
      print(paste("processing file", great.call.in, j)) # j tells which number processing
      ...
      for (k in 1:length(subsamps)) { 
        ...
        # Calculate RMS (sum of squares?) for each 10 ms long sample
        sum.square.list <- list ()
      ...
      SNR.dB <- 10 * log10(bins.minus.noise/noise.measure$noise.value)
    ...
    snr.dataframe<- do.call(rbind.data.frame, lapply(note.snr, data.frame, stringsAsFactors=FALSE))
    
    ## Check structure 
    str(snr.dataframe$snr)

#### Part 3 Quantile Regression and Model Selection code #### 

### Read in data file and check data structure

data.bw.r.calc <- read.csv("/Users/denasmacbook/Downloads/Performance-Constraints-master/performance.constraints.data.12.04.2017.csv",header=T)

### Subset all 1-sec bins with a signal-to-noise ratio greater than or equal to 10    
data.bw.r.calc <- subset(data.bw.r.calc, snr >= 10)

### Subset erroneous bandwidth measures  
data.bw.r.calc <- subset(data.bw.r.calc, bandwidth >= 300)
data.bw.r.calc <- subset(data.bw.r.calc, bandwidth <= 900)
data.bw.r.calc <- droplevels(data.bw.r.calc)
...
### Convert call names to site for color-coding
...
##  if VIF is > 10 multicollinearity is suggested

vif(lm(bandwidth ~ trill.rate + selection + playback + bin.new +trill.dur,data = data.bw.r.calc))
